...
```gherkin
Given 로그인하지 않은 사용자가 /를 연다
Then 첫 화면에서 로그인과 계정 만들기 행동을 볼 수 있다
And 제주 도움방, 여행자 커뮤니티, 참여 인증 메뉴를 볼 수 있다
And 대형 홍보 히어로나 긴 마케팅 섹션은 없다

When 제주 도움방 메뉴를 선택한다
Then /auth/login으로 이동한다
And 로그인 성공 후 /app/rooms/jeju로 이어질 안전한 next 경로가 보존된다

Given 로그인한 일반 사용자 또는 관리자가 /를 연다
Then 각각 /app 또는 /admin으로 이동한다
```

### E2E-031 비밀번호 재설정

```gherkin
Given 비밀번호 계정과 사용 가능한 메일 설정이 있다
When 등록된 이메일과 등록되지 않은 이메일로 재설정을 요청한다
Then 두 요청은 계정 존재 여부를 구분하지 않는 동일 응답을 반환한다

When 등록 계정의 30분 이내 재설정 링크로 새 비밀번호를 저장한다
Then 새 비밀번호로 로그인할 수 있다
And 기존 로그인 세션은 거부된다
And 같은 링크의 재사용은 거부된다
```

### E2E-032 소셜 로그인

```gherkin
Given Google, Kakao 또는 Apple 제공자가 설정되어 있다
When 사용자가 유효한 authorization code와 서명된 state로 돌아온다
Then 검증된 이메일의 로컬 계정이 생성되거나 기존 계정에 연결된다
And httpOnly 로그인 쿠키가 설정된다
And 안전한 앱 내부 next 경로로 이동한다

When state가 변조·만료되거나 이메일이 검증되지 않았다
