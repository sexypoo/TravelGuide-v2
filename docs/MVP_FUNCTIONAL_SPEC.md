# TravelGuide v2 — 2.5주 발표용 MVP 기능명세서
...
이 문서는 `TravelGuide v2`의 2.5주 발표용 MVP에서 **무엇을 만들고, 무엇을 만들지 않을지**를 고정한다.
...
특정 여행지의 **인증된 여행자**와 **인증된 현지인**만 입장할 수 있는 목적형 단체 채팅방을 제공한다. 여행자는 상황 질문을 올리고, 여러 현지인이 근거 유형을 표시해 답변한다. 여행자는 유용한 답변을 채택하고 질문을 해결 처리한다.
...
4. 질문-답변-채택-해결의 구조가 일반 오픈채팅보다 정보 탐색과 발표 시연에 적합하다.
...
4. 관리자가 두 신청을 승인한다.
5. 승인된 여행자가 제주 실시간 도움방에 입장해 상황 질문을 작성한다.
6. 서로 다른 두 브라우저의 승인된 현지인이 답변한다.
...
| 방 질문 피드 보기 | 불가 | 불가 | 가능 | 가능 | 가능 | 가능 |
| 질문 작성 | 불가 | 불가 | 가능 | 불가 | 가능 | 가능 |
| 답변 작성 | 불가 | 불가 | 불가 | 가능 | 가능 | 가능 |
...
- 사용자는 자신의 질문에 현지인 답변을 작성할 수 없다.
- 여행자는 자신의 질문만 채택·해결할 수 있다.
- 해결되거나 만료된 질문에는 새 답변을 작성할 수 없다.
- 관리자는 시연 및 운영을 위해 모든 방 콘텐츠를 열람할 수 있지만, 일반 사용자처럼 질문·답변을 작성하지 않는 것을 기본으로 한다.
...
- 한 현지인이 한 질문에 최대 3개 답변
- 답변자는 유효 인증 지역과 승인일 배지를 표시
...
- 재join 후 질문 목록과 현재 상세 질문을 REST로 재조회한다.
