...
### AUTH-006 계정 영구 삭제 — P1

- 로그인한 일반 사용자는 프로필에서 계정 삭제 범위와 복구 불가 안내를 확인한 뒤 삭제할 수 있다.
- 비밀번호 계정은 현재 비밀번호로 다시 인증하고, 모든 계정은 정확한 확인 문구를 입력한다.
- 관리자 계정은 공개 API로 삭제하지 않는다.
- 삭제 시 계정, 인증·여행 기록, 질문·답변·채팅·커뮤니티 콘텐츠, 신고, 찜, 같은 이메일의 사전예약 정보와 연결된 비공개 파일을 함께 제거한다.
- 다른 사용자의 답변·댓글이 소유자의 질문·게시물과 함께 연쇄 삭제될 때 해당 파일과 대상 신고도 고아 데이터로 남기지 않는다.
- Apple 로그인 refresh token은 암호화 저장하고 계정 삭제 전에 Apple에 폐기 요청한다.
- `/privacy`와 `/account-deletion`은 로그인 없이 열리며, 삭제 실행은 소유권 확인을 위해 웹 로그인을 요구한다.
...
- 로그인 상태면 사용자 홈 또는 관리자 홈으로 즉시 이동한다.
- 비로그인 상태에서는 로그인 CTA를 가장 먼저 제공하고 회원가입을 보조
  행동으로 제공한다.
- 제주 실시간 도움방, 여행자 커뮤니티, 참여 인증을 앱 메뉴 형태로 미리
  보여주고 선택 시 로그인 후 해당 내부 경로로 이어진다.
- 헤드라인 `여행이 틀어지는 순간, 지금 그곳을 아는 사람에게 묻다.`와
  인증 → 질문 → 여러 현지인 답변 순서는 첫 화면 안의 짧은 안내로 유지한다.
- 대형 홍보 히어로, 장식용 대화 목업, 기능별 마케팅 카드와 긴 스크롤형
  소개 섹션은 사용하지 않는다.
...
- 계정 삭제는 운영 DB의 계정 연결 데이터와 비공개 객체 저장소를 함께 정리하고, 공개 처리방침에 실제 범위와 경로를 설명한다.
