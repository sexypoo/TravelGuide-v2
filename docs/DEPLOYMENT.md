...
If Sign in with Apple is enabled, also add a sealed
`OAUTH_TOKEN_ENCRYPTION_KEY` generated by `openssl rand -hex 32`. Deploy the
database migration before enabling the provider; keep this key stable across
deployments because existing encrypted refresh tokens depend on it.
